...
const TraderProductDetails = ({ route, navigation }) => {
  const { product, type } = route.params;
  const [quantity, setQuantity] = useState('1');
  const [loading, setLoading] = useState(false);

  const priceKey = type === 'farmer' ? 'price' : 'price_per_kg';
  const quantityKey = type === 'farmer' ? 'quantity' : 'available_quantity';
  const productType = type === 'farmer' ? 'farmer_product' : 'trader_product';
  const sellerName = type === 'farmer' ? product.farmer_name : product.trader_name;
    ...
    <ScrollView style={styles.container}>
      {product.image_url ? (
        ...
        <View style={[styles.image, styles.placeholderImage]}>
          <Text style={styles.placeholderText}>{type === 'farmer' ? '🌾' : '🌱'}</Text>
        </View>
      )}
      ...
      <View style={styles.content}>
        ...
        <View style={styles.priceContainer}>
          <Text style={styles.price}>
            ₹{product[priceKey]}
            {type !== 'farmer' && <Text style={styles.perUnit}>/kg</Text>}
          </Text>
          <Text style={styles.available}>
            Available: {product[quantityKey]} {type !== 'farmer' ? 'kg' : 'units'}
          </Text>
        </View>
        ...
        <View style={styles.section}>
          ...
          <Text style={styles.phone}>
            📞 {type === 'farmer' ? product.farmer_phone : product.trader_phone || 'Not provided'}
          </Text>
        </View>
